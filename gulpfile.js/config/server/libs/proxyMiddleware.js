  ...
  return function (req, resp, next) {
    ...
    if (typeof options.route === 'string') {
      if (url === options.route) {
        url = '';
      } else if (nodeUrl.parse(url).pathname === options.route) {
        url = url.slice(options.route.length);
      } else if (options.isProxyToReal && url.slice(0, options.route.length) === options.route) {
        url = url.slice(options.route.length);
      } else {
        return next();
      }
...
function rewriteCookieHosts(existingHeaders, opts, applyTo, req) {
  ...
  const rewriteHostname = !opts.cookieRewrite ? 'localhost' : opts.cookieRewrite;
  ...
  for (let i = 0; i < existingCookies.length; i++) {
    let rewrittenCookie = existingCookies[i].replace(/(Domain)=[a-z\.-_]*?(;|$)/gi, `$1=${rewriteHostname}$2`); // eslint-disable-line no-useless-escape
