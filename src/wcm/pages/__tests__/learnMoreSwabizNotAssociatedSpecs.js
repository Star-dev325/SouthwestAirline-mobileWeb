...
describe('LearnMoreSwabizNotAssociated', () => {
  ...
  context('when loading wcm content', () => {
    ...
    it('should use goBack if there is history', () => {
      sinon.stub(RouterStore, 'getPrevState').returns({ routeState: 'route' });
      component.instance()._transitionToAirBookingPage();
      expect(goBackStub).to.have.been.called;
    });

    it('should fire the analytics satellite event with no company id associated', () => {
      component.instance().componentDidMount();

      expect(analyticsEventHelper.raiseSatelliteEvent).to.have.been.calledWith('no company id associated');
    });
  });
  ...
  function createComponent(props = {}) {
    ...
    return shallow(<LearnMoreSwabizNotAssociated {..._.merge({}, defaultProps, props)} />);
  }
});
