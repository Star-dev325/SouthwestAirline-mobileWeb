module.exports = {
  lowFareCalendarPage: {
    messages: [],
    disclaimerWithLinks:
      'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.',
    outboundPage: {
      header: {
        airportInfo: 'BZE - HOU',
        selectedDate: '2020-01-25'
      },
      lowFareCalendarDays: [
        {
          date: '2020-01-17',
          lowestPrice: {
            price: {
              amount: '28,356',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '11.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-18',
          lowestPrice: {
            price: {
              amount: '27,356',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-19',
          lowestPrice: {
            price: {
              amount: '29,356',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '12.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-20',
          lowestPrice: {
            price: {
              amount: '29,356',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '9.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-21',
          lowestPrice: {
            price: {
              amount: '19,356',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-22',
          lowestPrice: {
            price: {
              amount: '17,356',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-23',
          lowestPrice: {
            price: {
              amount: '29,356',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-24',
          lowestPrice: {
            price: {
              amount: '15,425',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-25',
          lowestPrice: {
            price: {
              amount: '14,814',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-26',
          lowestPrice: {
            price: {
              amount: '15,425',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-27',
          lowestPrice: {
            price: {
              amount: '9,620',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-28',
          lowestPrice: {
            price: {
              amount: '9,620',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-29',
          lowestPrice: {
            price: {
              amount: '9,620',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-30',
          lowestPrice: {
            price: {
              amount: '9,620',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-31',
          lowestPrice: {
            price: {
              amount: '5,992',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-02-01',
          lowestPrice: {
            price: {
              amount: '5,020',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-02-02',
          lowestPrice: {
            price: {
              amount: '5,992',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-02-03',
          lowestPrice: {
            price: {
              amount: '5,992',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-02-04',
          lowestPrice: {
            price: {
              amount: '5,992',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-02-05',
          lowestPrice: {
            price: {
              amount: '5,992',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-02-06',
          lowestPrice: {
            price: {
              amount: '5,992',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-02-07',
          lowestPrice: {
            price: {
              amount: '5,267',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-02-08',
          lowestPrice: {
            price: {
              amount: '5,267',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-02-09',
          lowestPrice: {
            price: {
              amount: '5,267',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        }
      ],
      _links: {
        previousLowFareCalendarPage: null,
        nextLowFareCalendarPage: {
          method: 'GET',
          href: '/v1/mobile-air-booking/page/flights/low-fare-calendar/products',
          query: {
            'origination-airport': 'BZE',
            'destination-airport': 'HOU',
            'departure-date': '2020-02-21'
          }
        }
      }
    },
    inboundPage: {
      header: {
        airportInfo: 'HOU - BZE',
        selectedDate: '2020-01-31'
      },
      lowFareCalendarDays: [
        {
          date: '2020-01-20',
          lowestPrice: {
            price: {
              amount: '29,356',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-21',
          lowestPrice: {
            price: {
              amount: '29,356',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-22',
          lowestPrice: {
            price: {
              amount: '29,356',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-23',
          lowestPrice: {
            price: {
              amount: '29,356',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-24',
          lowestPrice: {
            price: {
              amount: '15,425',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-25',
          lowestPrice: {
            price: {
              amount: '15,425',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-26',
          lowestPrice: {
            price: {
              amount: '15,425',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-27',
          lowestPrice: {
            price: {
              amount: '9,635',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-28',
          lowestPrice: {
            price: {
              amount: '9,635',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-29',
          lowestPrice: {
            price: {
              amount: '9,635',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-30',
          lowestPrice: {
            price: {
              amount: '9,635',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-01-31',
          lowestPrice: {
            price: {
              amount: '5,992',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-02-01',
          lowestPrice: {
            price: {
              amount: '5,992',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-02-02',
          lowestPrice: {
            price: {
              amount: '5,992',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-02-03',
          lowestPrice: {
            price: {
              amount: '5,992',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-02-04',
          lowestPrice: {
            price: {
              amount: '5,992',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-02-05',
          lowestPrice: {
            price: {
              amount: '5,992',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-02-06',
          lowestPrice: {
            price: {
              amount: '5,992',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-02-07',
          lowestPrice: {
            price: {
              amount: '5,267',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-02-08',
          lowestPrice: {
            price: {
              amount: '5,267',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-02-09',
          lowestPrice: {
            price: {
              amount: '5,267',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-02-10',
          lowestPrice: {
            price: {
              amount: '5,267',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-02-11',
          lowestPrice: {
            price: {
              amount: '5,267',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        },
        {
          date: '2020-02-12',
          lowestPrice: {
            price: {
              amount: '5,267',
              currencyCode: 'PTS',
              currencySymbol: null
            },
            pricePointsTax: {
              amount: '10.39',
              currencyCode: 'USD',
              currencySymbol: '$'
            }
          }
        }
      ],
      _links: {
        previousLowFareCalendarPage: {
          method: 'GET',
          href: '/v1/mobile-air-booking/page/flights/low-fare-calendar/products',
          query: {
            'origination-airport': 'HOU',
            'destination-airport': 'BZE',
            'departure-date': '2020-01-17'
          }
        },
        nextLowFareCalendarPage: {
          method: 'GET',
          href: '/v1/mobile-air-booking/page/flights/low-fare-calendar/products',
          query: {
            'origination-airport': 'HOU',
            'destination-airport': 'BZE',
            'departure-date': '2020-02-24'
          }
        }
      }
    },
    lowFareCalendarAnalytics: {
      passengercount: '1',
      currencycode: 'POINTS',
      triptype: 'RT',
      origindestination: 'BZEHOU',
      destinationreturn: 'HOUBZE',
      lowestpriceout: 'none',
      lowestpointsout: '5020',
      lowestpricereturn: 'none',
      lowestpointsreturn: '5020',
      datesout: '01172020|02092020',
      datesrtn: '01202020|02122020'
    },
    _meta: {
      isInternational: false
    },
    _links: {
      flightShoppingPage: {
        href: '/v1/mobile-air-booking/page/flights/products',
        method: 'GET',
        query: {
          currency: 'PTS',
          'origination-airport': 'BZE',
          'destination-airport': 'HOU',
          'number-adult-passengers': '1'
        }
      }
    }
  }
};
